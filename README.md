...
- 前台窗口变化时，自动切回英文输入法
- `Esc` / `Ctrl+[`：切到英文输入法
- 在普通桌面窗口里，单独按左 `Alt`：切到英文输入法
- 在普通桌面窗口里，单独按右 `Alt`：切到中文输入法
- `Alt + 非修饰键` 组合保持系统原本行为
- `Alt + 鼠标按键或滚轮` 组合保持系统原本行为
...
- 如果前台窗口已经全屏覆盖显示器（通常是游戏或全屏应用），左右 `Alt` 会直接透传，不再被 `imeswitch` 捕获
- `Alt` 只会在和非修饰键, 鼠标按键或鼠标滚轮组合时放行; `Shift` / `Ctrl` / `Win` / `CapsLock` 这类不会让它进入放行模式
